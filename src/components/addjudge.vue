...
export default {
  ...
  methods: {
    back() {
      ...
      if (content == "") {
        ...
        this.$parent.$parent.$options.parent.judge =
          this.$parent.$parent.$options.parent.judge + 1;
      ...
      if (content !== "") {
        this.$axios
          ...
          .then((res) => {
            console.log(res);
            this.$alert(
              "题目以及选项上传完毕，请点击选项按钮设定正确答案,若不点击,则默认正确答案为对",
              "新建题目",
              {
  ...
  mounted: function () {
    ...
    this.$axios
      .post(
        "http://106.55.103.200:8088/fire_ant_project/judge/addNullJudge?question=" +
          ""
      )
      .then((res) => {
        console.log(res);
