...
export default {
  ...
  methods: {
    ...
    newest() {
      this.tableData = [];
      this.$axios
        .get("http://106.55.103.200:8088/fire_ant_project/cover/getAllTArticleCover")
        .then((res) => {
    ...
    more() {
      this.$router.push("/menu/addtrainarticle");
    },
    ...
    del(id, index) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$axios
            .post(
              "http://106.55.103.200:8088/fire_ant_project/article/deleteTArticleByArticleId",
              { articleId: id }
            )
            .then((res) => {
              console.log(res);
